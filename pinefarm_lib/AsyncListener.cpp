...
void AsyncListener::Start(int PortNum)
{
	m_PortNum = PortNum;
	Start();
}

int AsyncListener::Start()
{
	DWORD Flags;
	WSADATA wsaData;
...
void CALLBACK AsyncListener::WorkerRoutine(DWORD Error, DWORD BytesTransferred, LPWSAOVERLAPPED Overlapped, DWORD InFlags)
{
...
DWORD WINAPI AsyncListener::ServerWorkerThread(LPVOID lpObject)
{
	AsyncListener *obj = (AsyncListener*)lpObject;
	HANDLE CompletionPort = (HANDLE)obj->GetCompletionPort();
	return 1;
}

HANDLE AsyncListener::GetCompletionPort()
{
